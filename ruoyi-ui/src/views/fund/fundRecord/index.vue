<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="基金代码" prop="fundId">
        <el-input
      ...
      <el-form-item label="类型" prop="type">
        <el-select v-model="queryParams.fundType" placeholder="请选择类型" clearable>
          <el-option
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态" clearable>
          <el-option
            v-for="dict in dict.type.fund_record_status"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['fund:fundRecord:add']"
        >新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['fund:fundRecord:edit']"
        >修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['fund:fundRecord:remove']"
        >删除
        </el-button>
      </el-col>
<!--      <el-col :span="1.5">-->
...
<!--          size="mini"-->
<!--          @click="handleExport"-->
<!--          v-hasPermi="['fund:fundRecord:export']"-->
<!--        >导出-->
<!--        </el-button>-->
<!--      </el-col>-->
    ...
    <el-table v-loading="loading" :data="fundRecordList" @selection-change="handleSelectionChange">
...
<!--      <el-table-column label="基金代码" align="center" prop="fundCode"/>-->
      <el-table-column label="基金类型" align="center" prop="fundType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.fund_type" :value="scope.row.fundType"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="购买金额" align="center" prop="amt"/>
      <el-table-column label="产生收益" align="center" prop="income"/>
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.fund_record_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <!--        </el-form-item>-->
        <!--        <el-form-item label="购买金额" prop="amt">-->
        <!--          <el-input v-model="form.amt" placeholder="请输入购买金额"/>-->
        <!--        </el-form-item>-->
        <el-form-item label="状态" prop="status">
...
<script>
import {addFundRecord, delFundRecord, getFundRecord, listFundRecord, updateFundRecord} from "@/api/fund/fundRecord";

export default {
  name: "FundRecord",
  dicts: ['fund_record_status', 'fund_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      listFundRecord(this.queryParams).then(response => {
        this.fundRecordList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getFundRecord(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFundRecord(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFundRecord(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除基金购买记录编号为"' + ids + '"的数据项？').then(function () {
        return delFundRecord(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('fund/fundRecord/export', {
        ...this.queryParams
      }, `fundRecord_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
