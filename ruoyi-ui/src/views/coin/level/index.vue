<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="levelList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="杠杆倍数" align="center" prop="level" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="杠杆倍数" prop="level">
          <el-input v-model="form.level" placeholder="请输入杠杆倍数" />
        </el-form-item>
        <el-form-item label="启用" prop="status">
...
<script>
import { listLevel, getLevel, delLevel, addLevel, updateLevel } from "@/api/coin/level";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        status: [
          { required: true, message: "启用不能为空", trigger: "change" }
        ],
        createTime: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除虚拟币杠杆编号为"' + ids + '"的数据项？').then(function() {
        return delLevel(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('coin/level/export', {
        ...this.queryParams
