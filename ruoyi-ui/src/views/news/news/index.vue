  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="展示" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择展示" clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="newsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="新闻类型" align="center" prop="type">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.news_type" :value="scope.row.type"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="显示时间" align="center" prop="showTime" width="180"/>
      <el-table-column label="图片" align="center" prop="imgurl" width="100">
        <template slot-scope="scope">
          <img style="width: 50px" :src="imagesUrl + scope.row.imgurl" alt="暂无"/>
        </template>
      </el-table-column>
      <el-table-column label="展示" align="center" prop="status">
        <template slot-scope="scope">
    ...
    <el-dialog :close-on-click-modal="false" :title="title" :visible.sync="open" width="1500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="展示" prop="status">
          <el-select v-model="form.status" placeholder="请选择展示">
            <el-option
              v-for="dict in dict.type.yes_no"
              :key="dict.value"
              :label="dict.label"
...
<script>
import {addNews, delNews, getNews, listNews, updateNews} from "@/api/news/news";

export default {
  name: "News",
  dicts: ['news_source', 'news_type', 'yes_no'],
  data() {
    return {
